...
export const USE_GRAPHQL = true as const;
// export const USE_GRAPHQL = false as const;
...
export const APP_CONFIG = {
  ...
  API_BASE_URL_CMS: import.meta.env.VITE_API_BASE_URL_CMS || "/api/v1",
  API_BASE_URL_CRM: import.meta.env.VITE_API_BASE_URL_CRM || "/api/v1",
...
/** Absolute session cap, measured from login. Not extendable, not reset by activity. */
export const SESSION_TIMEOUT_TIMER = 480; // min.
/** Lead time before a deadline at which the warning modal appears. */
export const SESSION_TIMEOUT_WARNING = 1; // min.
/** Lead time before token expiry at which a silent refresh is attempted.
 *  Must stay greater than SESSION_TIMEOUT_WARNING, otherwise the modal would
 *  appear on every healthy refresh cycle. */
export const SESSION_REFRESH_LEAD = 5; // min.
/** How often the absolute cap is re-checked against the wall clock. */
export const SESSION_HARD_CAP_CHECK_INTERVAL = 30 * 1000; // ms.
/** Minimum gap between UPDATE_LAST_ACTIVITY dispatches. Raw DOM activity events fire
 *  dozens of times a second and every dispatch re-renders all auth context consumers. */
export const ACTIVITY_DISPATCH_THROTTLE = 30 * 1000; // ms.
/** localStorage key holding the absolute cap deadline, so it survives a reload. */
export const SESSION_HARD_EXPIRY_KEY = "session_hard_expiry" as const;
