...
interface BroadcastToastItem {
  id: number;
  tone: "success" | "error";
  message: string;
}

interface BroadcastToastStackProps {
  toasts: BroadcastToastItem[];
  onDismiss: (id: number) => void;
}

const toneClassMap: Record<
  BroadcastToastItem["tone"],
  {
...
const BroadcastToastStack = ({
  toasts,
  onDismiss,
}: BroadcastToastStackProps) => {
  if (toasts.length === 0) {
    return null;
  }
...
export default BroadcastToastStack;
