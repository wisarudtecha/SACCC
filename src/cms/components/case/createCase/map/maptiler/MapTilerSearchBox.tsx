// Place search for the MapTiler map.
//
// Exists for the same reason LongdoSearchBox does: MapTiler ships no search
// widget, and the ArcGIS map gets its search box from the SDK. Written as an app
// control so it follows the app's dark mode and translations the way
// BasemapSwitcher already does.
//
// It reports a picked place and nothing else. What that MEANS - move the pin, or
// just navigate there in view-only mode - is the map's decision.
//
// The two anti-regression guards from LongdoSearchBox are carried over verbatim:
// `t` is held in a ref (useTranslation rebuilds it every render, so it must
// never be an effect dependency), and `setCandidates` is written idempotently
// so a stray re-run cannot start a render loop.
import { memo, useCallback, useEffect, useRef, useState } from "react";
import { Loader2, Search, X } from "lucide-react";
import { useTranslation } from "@/core/hooks/useTranslation";
import { searchPlaces, type PlaceCandidate } from "../services/maptilerGeocode";

interface MapTilerSearchBoxProps {
...
/** Same floor the ArcGIS Search widget and LongdoSearchBox use. */
const SEARCH_MIN_CHARACTERS = 3;

/** The widget debounces internally; this is our equivalent. */
const SEARCH_DEBOUNCE_MS = 350;

function MapTilerSearchBoxBase({ onSelect, onError, compact = false }: MapTilerSearchBoxProps) {
  ...
  // `t` is a NEW function every render (useTranslation builds its closure
  // inline), so it must never be a dependency of the effect below - the effect
  ...
  useEffect(() => {
    ...
    if (trimmed.length < SEARCH_MIN_CHARACTERS) {
      ...
      // a new array identity every time and defeat its bail-out.
    ...
    const timeoutId = setTimeout(() => {
      searchPlaces(trimmed, language)
        ...
        .catch((error: unknown) => {
          ...
          console.error("MapTiler place search failed", error);
          setCandidates((previous) => (previous.length === 0 ? previous : []));
...
export const MapTilerSearchBox = memo(MapTilerSearchBoxBase);
MapTilerSearchBox.displayName = "MapTilerSearchBox";

export default MapTilerSearchBox;
