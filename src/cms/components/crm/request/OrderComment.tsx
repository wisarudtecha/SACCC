// OrderComment.tsx — Comment component for Order/Inventory requests
// Mirrors the case Comment.tsx pattern but uses orderComment API
...
} from "@/cms/store/api/orderComment";
import type { OrderComment } from "@/cms/types/orderComment";
...
export const OrderComments: React.FC<OrderCommentsProps> = ({
    isOpen,
    orderId,
    isModal = false,
}) => {
    ...
    const [commentsData, setCommentsData] = useState<OrderComment[]>([]);

    const { data: fetchedComments, isLoading: isFetchingComments, error: fetchError } = useOrderCommentListQuery(
        orderId,
        {
    ...
    const [createOrderComment, { isLoading: isAddingComment, error: addCommentError }] = useCreateOrderCommentMutation();
    ...
    const renderContent = () => {
                ...
                <div className="flex justify-center items-center h-full">
                    <div className="text-center">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 text-sm font-medium">
                            {t("case.sop_card.no_comment") || "No comments yet"}
                        </p>
                    </div>
                </div>
            );
        ...
        return commentsData.map((comment: OrderComment) => (
            <div
                key={comment.id}
            ...
            >
                ...
                <div className="flex space-x-2 justify-between">
                    <p className="text-sm text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap ml-2">{comment.remark}</p>
                </div>
            </div>
        ));
    };
